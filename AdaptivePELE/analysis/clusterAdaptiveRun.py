...
def assesClusterConvergence(df, num_clusters, traj_name = "trajectory_", topology=None):
    for i in range(num_clusters):
        ...
        BEs =  maximum["Binding Energy"].values 
        ...
        for j, (epoch, traj, step, be) in enumerate(zip(epochs, trajs, steps, BEs)):
            trajectory = "{}/{}{}.xtc".format(int(epoch), traj_name, int(traj)) if topology else "{}/{}{}.pdb".format(int(epoch), traj_name, int(traj))
            write_snapshot(int(step), trajectory, os.path.join(path, "Cluster_{}_{}.pdb".format(i, be)), topology=topology)
